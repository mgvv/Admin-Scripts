...
function net_info(){
devices=$(ss -i | cut -d" " -f1 | egrep -v "^Kernel|Iface|lo")
...
4 )
	...
	case $deleteuserays in
		y | Y )
			if id "$deletethisuser" &>/dev/null; then
  userdel "$deletethisuser"
  log_action "Deleted user $deletethisuser"
else
  echo "User $deletethisuser does not exist."
fi && echo "${txtgrn}User $deletethisuser deleted successfully." | center || echo "${txtred}Failed to delete user. Please check username and try again.${txtrst}" | centerwide
		;;
		n | N )
...
2 )
	...
	if [[ -f "$rmfile" ]]; then
  rm -i "$rmfile"
  log_action "Deleted file $rmfile"
else
  echo "File $rmfile does not exist."
fi && echo "${txtgrn}File removed successfully.${txtrst}" | center || echo "${txtred}File removal failed.${txtrst}" | center
	echo "" && echo "${txtcyn}(press ENTER to continue)${txtrst}" | center && read
;;
...
if [[ -e "$symlinfile" ]]; then
  ln -s "$symlinfile" "$symloutfile"
  if [[ -L "$symloutfile" ]]; then
    echo "Symbolic link created successfully at $symloutfile"
    log_action "Created symbolic link from $symlinfile to $symloutfile"
  else
    echo "Failed to create symbolic link."
  fi
else
  echo "Input file $symlinfile does not exist."
fi
